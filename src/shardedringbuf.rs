...
impl<T> InnerRingBuffer<T> {
    ...
    fn new(capacity: usize) -> Self {
        let (s, r) = kanal::bounded::<T>(capacity);
        InnerRingBuffer {
            sender: s,
            receiver: r
        }
...
impl<T> ShardedRingBuf<T> {
    ...
    fn dequeue_in_shard(&self, shard_ind: usize) -> T {
        ...
        // }
        inner.receiver.recv().expect("Guaranteed value here")
    }
    ...
    pub fn is_shard_full(&self, shard_ind: usize) -> bool {
        ...
        // let (enq_ind, deq_ind) = (inner.enqueue_index.get(), inner.dequeue_index.get());
        // let jobs = enq_ind.wrapping_sub(deq_ind);
        inner.sender.len() == inner.sender.capacity()
    }
